...
public class SingleGameEngine implements SingleGameEngineInterface {
	...
	public MoveResult makeMove(int x, int y, boolean isRed) {return makeMove(x, y, (isRed) ? MoveType.RED : MoveType.BLUE);}

	public MoveResult makeMove(int x, int y, MoveType moveType) {
		if (!canMakeMove(x, y)) {return MoveResult.ERROR;}
			
		lastMoveInfo = new MoveInfo(x, y, moveType, MoveResult.ERROR);
		Dot lastPlacedDot = field[x][y];
		lastDot = lastPlacedDot;
		Surrounding ctrlSurrounding=(lastPlacedDot.isCtrl()) ? field[x][y].masterSurrounding : null;
		...
		if (ctrlSurrounding == null) { // there was no Ctrl surrounding
			...
			if (surroundedSomething) {lastMoveInfo.moveResult = MoveResult.GOOD;return lastMoveInfo.moveResult;} 
			else {lastMoveInfo.moveResult = MoveResult.NOTHING;return lastMoveInfo.moveResult;}
		}
			...
			if (ctrlSurrounding.isEnemyCtrlFor(moveType)) { // dot was placed
				...
				if (surroundedSomething == false) {
					...
					ctrlSurrounding.type =(moveType == MoveType.BLUE) ? SurroundingType.RED: SurroundingType.BLUE;

					// getSurroundingType_Enemy(moveType);
					for (int indexOfDot = ctrlSurrounding.capturedPoints.size() - 1; indexOfDot >= 0; indexOfDot--) {
						DotAbstract abstractDot =ctrlSurrounding.capturedPoints.get(indexOfDot);
		...
		private boolean trySurround(Dot startDot, int startDirection,MoveType moveType, Surrounding outerCtrlSurrounding) {
			{
		...
		public int getSizeX() {return sizeX - 2;}

		public int getSizeY() {return sizeY - 2;}

		public DotType getDotType(int x, int y) {return field[x][y].type;}

		public DotAbstract getLastDot() {return lastDot;}
		...
		DotType getDotType(MoveType moveTypeFromUser) {
			if (moveTypeFromUser == MoveType.BLUE) {return DotType.BLUE;} 
			else {return DotType.RED;}
		}

		SurroundingType getSurroundingType(MoveType moveType, boolean enemyInside) {
			if (enemyInside)return (moveType == MoveType.BLUE) ? SurroundingType.BLUE: SurroundingType.RED;	
			else return (moveType == MoveType.BLUE) ? SurroundingType.BLUE_CTRL: SurroundingType.RED_CTRL;
		}

		class Surrounding extends SurroundingAbstract {
			boolean wasDestroyed = false;

			boolean isCtrl(){return (type == SurroundingType.BLUE_CTRL)|| (type == SurroundingType.RED_CTRL);}
			...
			public Surrounding() {path = new ArrayList<DotAbstract>();capturedPoints = new ArrayList<DotAbstract>();}
		}

		class MoveInfo extends MoveInfoAbstract {

			public MoveInfo(int coordX, int coordY, MoveType moveType,MoveResult moveResult) {
				this.coordX = coordX;
				this.coordY = coordY;
		...
		class Dot extends DotAbstract {
			...
			boolean dotTypeIsIn(DotType element, DotType... set) {
				for (int i = 0; i < set.length; i++)if(set[i] == element)return true;
				return false;
			}

			boolean hasType(DotType... set) {return dotTypeIsIn(this.type, set);}

			boolean isEmptyToPlace() {return hasType(DotType.EMPTY, DotType.RED_CTRL, DotType.BLUE_CTRL);}
			...
			boolean isUneatableFor(MoveType moveType) {	return false;}
			...
			boolean isCtrl() {return type == DotType.BLUE_CTRL || type == DotType.RED_CTRL;	}

			Dot getNeighbour(int direction) {
				direction %= DIRECTIONS; // the change of this variable is local in
				// java
				if (direction < 0)direction = direction + DIRECTIONS;
				int dx, dy;
				switch (direction) {
					...
					default:throw new UnsupportedOperationException("unknown direction");
				}
			...
			int rotateClockwise(int previousDirection, MoveType moveType) {
				...
				while (this.getNeighbour(newDirection).isSurrBuilder(moveType) == false) {newDirection += 1;}
				return newDirection;
			}
			...
			int getOppositeDirection(int direction) {return 4 + direction;}
			...
			void eat(Surrounding newSurrounding, Surrounding outerCtrlSurrounding) {
				...
				if (newSurrounding.isCtrl() == false)this.height += 1;
//				System.out.println("501 newSurr.isCtrl()=" + newSurrounding.isCtrl());
//				System.out.println("502 masterSurr=" + masterSurrounding);
				if (this.masterSurrounding == null) {this.masterSurrounding = newSurrounding;newSurrounding.capturedPoints.add(this);} 
					...
					} else {newSurrounding.capturedPoints.add(this);}
				}

				// <changing Dot types>
				switch (newSurrounding.type) {
					case BLUE:
						if (this.hasType(DotType.BLUE)) {type = DotType.BLUE_TIRED;
						} else if (this.hasType(DotType.RED_EATED_BLUE)) {redScore -= 1;type = DotType.BLUE_TIRED;
						} else if (this.hasType(DotType.RED)) {blueScore += 1;newSurrounding.firstCapturedEnemy = this;type = DotType.BLUE_EATED_RED;
						} else if (this.hasType(DotType.RED_TIRED)) {blueScore += 1;type = DotType.BLUE_EATED_RED;
						} else if (this.hasType(DotType.BLUE_CTRL, DotType.EMPTY,
								DotType.RED_CTRL, DotType.RED_EATED_EMPTY)) {type = DotType.BLUE_EATED_EMPTY;}
						break;
					case RED:
						if (this.hasType(DotType.RED)) {type = DotType.RED_TIRED;
						} else if (this.hasType(DotType.BLUE_EATED_RED)) {blueScore -= 1;type = DotType.RED_TIRED;
						} else if (this.hasType(DotType.BLUE)) {redScore += 1;newSurrounding.firstCapturedEnemy = this;type = DotType.RED_EATED_BLUE;
						} else if (this.hasType(DotType.BLUE_TIRED)) {redScore += 1;type = DotType.RED_EATED_BLUE;
						} else if (this.hasType(DotType.RED_CTRL, DotType.EMPTY,
								DotType.BLUE_CTRL, DotType.BLUE_EATED_EMPTY)) {type = DotType.RED_EATED_EMPTY;}
						break;
					case BLUE_CTRL:if (this.hasType(DotType.EMPTY)) {type = DotType.BLUE_CTRL;}break;
					case RED_CTRL:if (this.hasType(DotType.EMPTY)) {type = DotType.RED_CTRL;}break;
					default:throw new UnsupportedOperationException("unknown surrounding type");
				}
				// <changing Dot types>
			}
