...
public class PointsAIEngine implements Gui2Ai_Interface {
	...
	public PointsAIEngine(Ai2Gui_Interface gui, int sizeX, int sizeY) {
		this.gui = gui;
		this.engine = new SingleGameEngine(sizeX, sizeY);

		try {
			pointsAI = new com.google.sites.priymakpoints.pointsai.pointsAI_1_10.PointsAI();
		} catch (Exception e1) {
			JOptionPane.showMessageDialog(null, "Невозможно запустить ИИ!", "Ошибка", JOptionPane.ERROR_MESSAGE);
		}
	}
