...
public String toString() {
	switch (this) {
		...
		case WALL_SIDE:		return "wst";
		case WALL_CORNER:	return "wct";
		case BLUE_SURROUND:	return "bst";
		case RED_SURROUND:	return "rst";
		case RED_WRONG:		return "rwt";
...
public boolean isSquare(){
	switch (this) {
		...
		case WALL_SIDE:		return true;
		case WALL_CORNER:	return true;
		case BLUE_SURROUND:	return true;
		case RED_SURROUND:	return true;
		case RED_WRONG:		return true;
...
public static TemplateType getTemplateType(String str){	
	...
	if(str.equals(TemplateType.WALL_CORNER.toString()))return TemplateType.WALL_CORNER;
...
public String getContent(PointsAIGame game,int x,int y,boolean isVertical){try{
	...
	switch (this) {
		case SQUARE_SIDE: case WALL_SIDE:
			if(x>(sizeX-7)&y>5&y<(sizeY-5)){
				for(int j=y-5;j<y+4;j++){
					for(int i=31;i<39;i++)if(i==(x-1)&j==(y-1)&this==SQUARE_SIDE)content+="E";else content+=fieldState[i][j];
					content+="LOOOO";
				}
			}
			if(x<8&y>5&y<(sizeY-5)){
				for(int j=y-5;j<y+4;j++){
					content+="L";
					for(int i=0;i<8;i++)if(i==(x-1)&j==(y-1)&this==SQUARE_SIDE)content+="E";else content+=fieldState[i][j];
					content+="OOOO";
				}
			}
			if(y>(sizeY-7)&x>5&x<(sizeX-5)){
				for(int j=(sizeY-8);j<sizeY;j++){
					for(int i=x-5;i<x+4;i++)if(i==(x-1)&j==(y-1)&this==SQUARE_SIDE)content+="E";else content+=fieldState[i][j];
					content+="OOOO";
					}
		...
		case SQUARE_CORNER:	case WALL_CORNER:
				if(x<8&y<8){content+="LLLLLLLLLOOOO";for(int j=0;j<8;j++){content+="L";for(int i=0;i<8;i++)content+=fieldState[i][j];content+="OOOO";}}
				if(x<8&y>(sizeY-7)){for(int j=(sizeY-8);j<sizeY;j++){content+="L";for(int i=0;i<8;i++)content+=fieldState[i][j];content+="OOOO";}content+="LLLLLLLLLOOOO";}
				if(x>(sizeX-7)&y>(sizeY-7)){for(int j=(sizeY-8);j<sizeY;j++){for(int i=(sizeX-8);i<sizeX;i++)content+=fieldState[i][j];content+="LOOOO";}content+="LLLLLLLLLOOOO";}
				if(x>(sizeX-7)&y<8){content+="LLLLLLLLLOOOO";for(int j=0;j<8;j++){for(int i=(sizeX-8);i<sizeX;i++)content+=fieldState[i][j];content+="LOOOO";}}
				return content;
		...
		case WALL: case SQUARE:	case BLUE_SURROUND: case RED_SURROUND: case RED_WRONG:
			for(int j=0;j<9;j++){
				for(int i=0;i<9;i++){
